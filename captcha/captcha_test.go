...
func TestDefaultCaptchaTypeIsDigit(t *testing.T) {
	store := NewMockCaptchaStore()
	service := NewDefaultImageCaptchaService(store, CaptchaOption{})

	resp, err := service.GenerateImageCaptcha(context.Background(), 0, 0)
	if err != nil {
		t.Fatalf("GenerateImageCaptcha failed: %v", err)
	}

	if len(resp.Value) != 4 {
		t.Fatalf("Expected default captcha length 4, got %d", len(resp.Value))
	}
	assertCaptchaValueFromSource(t, resp.Value, CaptchaSourceDigits)
}

func TestStringCaptchaWithDefaultSource(t *testing.T) {
	store := NewMockCaptchaStore()
	service := NewDefaultImageCaptchaService(store, CaptchaOption{
		Type:   CaptchaTypeString,
		Length: 8,
	})
	...
	if len(resp.Value) != 8 {
		t.Fatalf("Expected captcha length 8, got %d", len(resp.Value))
	}
	assertCaptchaValueFromSource(t, resp.Value, CaptchaSourceAlphaNumeric)
}

func TestStringCaptchaWithCustomSource(t *testing.T) {
	store := NewMockCaptchaStore()
	service := NewDefaultImageCaptchaService(store, CaptchaOption{
		Length:          6,
		CharacterSource: CaptchaSourceLetters,
	})
	...
	if len(resp.Value) != 6 {
		t.Fatalf("Expected captcha length 6, got %d", len(resp.Value))
	}
	assertCaptchaValueFromSource(t, resp.Value, CaptchaSourceLetters)

	isValid, err := service.VerifyCaptcha(context.Background(), resp.ID, strings.ToLower(resp.Value))
	if err != nil {
		t.Fatalf("VerifyCaptcha failed: %v", err)
	}
	if !isValid {
...
func TestUnsupportedCaptchaType(t *testing.T) {
	store := NewMockCaptchaStore()
	service := NewDefaultImageCaptchaService(store, CaptchaOption{
		Type: CaptchaType("unknown"),
	})

	_, err := service.GenerateImageCaptcha(context.Background(), 0, 0)
	if err == nil {
		t.Fatal("Expected unsupported captcha type error")
	}
}

func assertCaptchaValueFromSource(t *testing.T, value, source string) {
	t.Helper()
	...
	for _, char := range value {
		if !strings.ContainsRune(source, char) {
			t.Fatalf("Expected captcha value %q to use only source %q, got %q", value, source, char)
		}
	}
}
